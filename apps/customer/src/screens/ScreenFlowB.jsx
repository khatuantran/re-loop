...
const TextInput = (p) => (
  <input
    {...p}
    className="w-full h-11 px-3.5 text-[13.5px] rounded-lg border border-slate-200 focus:border-emerald-500 focus:ring-2 focus:ring-emerald-100 outline-none"
  />
);
...
const Select = ({ value, options }) => (
  <div className="relative">
    <select
      defaultValue={value}
      className="w-full h-11 px-3.5 pr-9 text-[13.5px] rounded-lg border border-slate-200 focus:border-emerald-500 outline-none appearance-none bg-white"
    >
      {options.map((o) => (
...
const ModeCard = ({ id, mode, setMode, icon: I, title, tagline, fitFor, price, features, accent }) => {
  const active = mode === id;
  const tone =
    accent === 'amber'
      ? {
          ring:
            'border-amber-400 bg-amber-50/40 shadow-[0_18px_40px_-18px_rgba(251,191,36,0.4)]',
      ...
      : {
          ring:
            'border-emerald-500 bg-emerald-50/40 shadow-[0_18px_40px_-18px_rgba(16,185,129,0.4)]',
  ...
  return (
    <button
      onClick={() => setMode(id)}
      className={`relative w-full text-left rounded-2xl border-2 p-5 md:p-6 transition-all ${
        active ? tone.ring : 'border-slate-200 bg-white hover:border-slate-300'
      }`}
    >
      <div className="flex items-start gap-3 md:gap-4 mb-4">
        <div
          className={`w-12 h-12 md:w-14 md:h-14 rounded-xl flex items-center justify-center shrink-0 ${tone.iconBg}`}
        >
          <I size={26} />
        </div>
        <div className="flex-1 min-w-0">
          <div className="flex items-center gap-2 mb-1 flex-wrap">
            <span className="text-[18px] md:text-[20px] font-extrabold text-slate-900">
              {title}
            </span>
            {id === 'speed' && (
              <Pill tone="green" className="!py-0.5 !text-[10px]">
                Phổ biến
              </Pill>
            )}
          </div>
          <div className={`text-[12.5px] md:text-[13px] font-bold ${tone.tag}`}>{tagline}</div>
        </div>
        <div
          className={`w-7 h-7 rounded-full border-2 flex items-center justify-center shrink-0 ${
            active ? tone.check : 'bg-white border-slate-300'
          }`}
        >
          {active && <IconCheck size={16} className="text-white" />}
        </div>
      </div>
      <div className="text-[12.5px] md:text-[13px] text-slate-600 mb-4 leading-relaxed">
        {fitFor}
      </div>
      <div className="space-y-2 mb-4">
        {features.map((f) => (
          <div key={f} className="flex items-center gap-2 text-[12.5px] text-slate-700">
            <IconCheck size={14} className={tone.price} />
            {f}
          </div>
        ))}
      </div>
...
export default function ScreenFlowB() {
  const navigate = useNavigate();
  const [mode, setMode] = useState('speed');
  const [stage, setStage] = useState('select');
  const [photos, setPhotos] = useState([true, true, false, false, false]);
  const [floor, setFloor] = useState('Lầu 3');
  ...
  return (
    <div className="px-4 md:px-8 py-4 md:py-8 max-w-[1280px] mx-auto">
      <div className="flex flex-col sm:flex-row sm:items-end sm:justify-between gap-3 mb-5 md:mb-6">
        ...
        {stage === 'form' && (
          <Btn variant="ghost" size="md" onClick={() => setStage('select')}>
            ← Đổi chế độ
          </Btn>
        )}
      </div>
          ...
          <Card className="p-0 overflow-hidden mb-5 md:mb-6">
            <div className="px-5 py-3 bg-slate-50 border-b border-slate-200 flex items-center justify-between">
              ...
              <div className="text-[11px] text-slate-400">
                Đang chọn:{' '}
                <b className="text-slate-700">{mode === 'speed' ? 'Speed' : 'Auction'}</b>
              </div>
            </div>
              ...
              <table className="w-full text-[13px] min-w-[420px]">
                <thead>
                  <tr className="border-b border-slate-100">
                    <th className="py-3 px-4 md:px-5 text-left text-[11px] uppercase tracking-wider text-slate-400 font-bold">
                      Tiêu chí
                    </th>
                    <th
                      className={`py-3 px-4 md:px-5 text-center text-[12px] font-bold ${
                        mode === 'speed' ? 'text-emerald-700 bg-emerald-50/40' : 'text-slate-700'
                      }`}
                    >
                      Speed
                    </th>
                    <th
                      className={`py-3 px-4 md:px-5 text-center text-[12px] font-bold ${
                        mode === 'auction' ? 'text-amber-700 bg-amber-50/40' : 'text-slate-700'
                      }`}
                    >
                      Auction
                    </th>
                ...
                <tbody>
                  {[
                    ...
                    ['Hủy đơn', 'Miễn phí <1ph', 'Miễn phí <5ph'],
                  ].map(([k, a, b], i) => (
                    <tr
                      key={k}
                      className={`border-b border-slate-50 last:border-0 ${i % 2 ? 'bg-slate-50/30' : ''}`}
                    >
                      <td className="py-3 px-4 md:px-5 text-slate-500 font-medium">{k}</td>
                      <td
                        className={`py-3 px-4 md:px-5 font-bold text-center ${
                          mode === 'speed' ? 'text-emerald-700 bg-emerald-50/30' : 'text-slate-700'
                        }`}
                      >
                        {a}
                      </td>
                      <td
                        className={`py-3 px-4 md:px-5 font-bold text-center ${
                          mode === 'auction' ? 'text-amber-700 bg-amber-50/30' : 'text-slate-700'
                        }`}
                      >
                        {b}
                      </td>
          ...
          <div className="md:col-span-8 space-y-5">
            <Card className="p-5 md:p-6">
              <Pill tone={mode === 'speed' ? 'green' : 'amber'} className="mb-4">
                {mode === 'speed' ? <IconZap size={11} /> : <IconGavel size={11} />}
                {mode === 'speed' ? 'Speed Mode' : 'Auction Mode'}
              </Pill>
              <div className="space-y-5">
                <div>
                  <Label>Mô tả đồ cần bán</Label>
                  <TextInput defaultValue="Tủ lạnh Sanyo 150L, hỏng máy nén" />
                </div>
                    ...
                    {photos.map((p, i) => (
                      <button
                        ...
                        className={`aspect-square rounded-xl border-2 flex items-center justify-center text-slate-400 transition-colors ${
                          p
                            ? 'photo-ph border-emerald-200'
                            : 'border-dashed border-slate-300 bg-white hover:border-emerald-300'
                        }`}
                      >
                        {p ? <IconCheck size={20} className="text-emerald-600" /> : <IconPlus size={18} />}
                      </button>
                    ))}
                  </div>
                </div>
                ...
                <div>
                  ...
                  <textarea
                    rows="3"
                    defaultValue="Chung cư có thang máy, gọi trước 10 phút khi đến."
          ...
          <div className="md:col-span-4 space-y-4">
            <Card className="p-5">
              ...
              <button
                ...
                className="w-full flex items-center justify-between p-3 rounded-xl bg-slate-50 border border-slate-200 mb-3"
              >
                ...
                <div
                  className={`w-11 h-6 rounded-full p-0.5 transition-colors shrink-0 ${
                    needsHelp ? 'bg-emerald-500' : 'bg-slate-300'
                  }`}
                >
                  <div
                    className={`w-5 h-5 rounded-full bg-white transition-transform ${needsHelp ? 'translate-x-5' : ''}`}
                  />
                </div>
              </button>
            </Card>
            ...
            <Card className="p-5">
              ...
              <div className="space-y-2">
                <div className="flex justify-between text-[13px]">
                  <span className="text-slate-500">Cân nặng dự kiến</span>
                  <span className="font-bold tabular-nums">~40 kg</span>
                </div>
                <div className="flex justify-between text-[13px]">
                  <span className="text-slate-500">Giá sàn</span>
                  <span className="font-bold tabular-nums">720.000 ₫</span>
                </div>
                {needsHelp && (
                  <div className="flex justify-between text-[13px]">
                    <span className="text-slate-500">Phụ phí khuân</span>
                    <span className="font-bold tabular-nums">+10.000 ₫</span>
                  </div>
                )}
            ...
            <Btn
              ...
              size="lg"
              className="w-full"
              onClick={() => navigate(mode === 'speed' ? '/tracking' : '/auction')}
              iconRight={IconArrowRight}
            >

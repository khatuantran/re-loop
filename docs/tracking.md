...
- 2026-04-27: **Bug fix Netlify config** — đổi `publish = "apps/<name>/dist"` → `publish = "dist"` trong cả 4 file `netlify.toml`. Khi `base` được set trong netlify.toml, Netlify treat `publish` relative to base; nếu để absolute path sẽ bị nhân đôi (`apps/collector/apps/collector/dist`). Tạo `docs/deploy-netlify.md` — hướng dẫn step-by-step bằng tiếng Việt với form fields đối chiếu Netlify UI, troubleshooting table. Update `architecture.md` ví dụ + `README.md` link sang deploy guide.
